...
try:
    ...
    HAS_BPY = False
    import trimesh as _trimesh_fallback
    print("NOTE: bpy not available - OBJ->GLB will use trimesh fallback")
import math
import numpy as np
...
def convert_obj_to_glb(
    ...
    """Convert OBJ file to GLB format. Uses Blender if available, trimesh otherwise."""
    if HAS_BPY:
        try:
        ...
        try:
            scene = _trimesh_fallback.load(obj_path, process=False, force="scene")
            scene.export(glb_path, file_type="glb")
            print(f"OBJ->GLB conversion done via trimesh: {glb_path}")
            return True
        except Exception as e:
            print(f"Trimesh OBJ->GLB fallback failed: {e}")
            return False
