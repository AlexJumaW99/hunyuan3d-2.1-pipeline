...
# === LOW VRAM MODEL SWAPPING HELPERS (added by 16GB patch) ===
def offload_shape_to_cpu():
    ...
    try:
        if hasattr(i23d_worker, 'model') and i23d_worker.model is not None:
            ...
            for name in dir(i23d_worker):
                attr = getattr(i23d_worker, name, None)
                if attr is not None and isinstance(attr, torch.nn.Module):
                    attr.to('cpu')
        torch.cuda.empty_cache()
        import gc; gc.collect()
        print("[LOW_VRAM] Shape model offloaded to CPU, VRAM freed")
    except Exception as e:
        print(f"[LOW_VRAM] Warning offloading shape: {e}")

def restore_shape_to_gpu():
    ...
    try:
        if hasattr(i23d_worker, 'model') and i23d_worker.model is not None:
            ...
            for name in dir(i23d_worker):
                ...
                if attr is not None and isinstance(attr, torch.nn.Module):
                    attr.to('cuda')
        print("[LOW_VRAM] Shape model restored to GPU")
    except Exception as e:
        print(f"[LOW_VRAM] Warning restoring shape: {e}")

def move_texture_to_gpu():
    """[MMGP] No-op: mmgp handles GPU placement automatically during inference.
    We only need to move dino_v2 to GPU since mmgp doesn't manage it.
    """
    global tex_pipeline
    try:
        ...
        if mv is not None:
            mv.device = 'cuda'
            # dino_v2 is not part of the diffusion pipeline, so mmgp doesn't manage it
            if hasattr(mv, 'dino_v2') and mv.dino_v2 is not None:
                mv.dino_v2 = mv.dino_v2.to('cuda')
        torch.cuda.empty_cache()
        vram_free = torch.cuda.mem_get_info()[0] / 1024**3
        print(f"[MMGP] Texture ready (mmgp manages GPU placement). Free VRAM: {vram_free:.1f} GB")
    except Exception as e:
        print(f"[MMGP] Warning in move_texture_to_gpu: {e}")

def move_texture_to_cpu():
    """[MMGP] Minimal cleanup: mmgp manages the pipeline, we just handle dino_v2."""
    global tex_pipeline
    try:
        mv = tex_pipeline.models.get("multiview_model")
        if mv is not None:
            mv.device = 'cpu'
            # Move dino_v2 back to CPU (not managed by mmgp)
            if hasattr(mv, 'dino_v2') and mv.dino_v2 is not None:
                mv.dino_v2 = mv.dino_v2.to('cpu')
        torch.cuda.empty_cache()
        import gc; gc.collect()
        vram_free = torch.cuda.mem_get_info()[0] / 1024**3
        print(f"[MMGP] Texture cleanup done. Free VRAM: {vram_free:.1f} GB")
    except Exception as e:
        print(f"[MMGP] Warning in move_texture_to_cpu: {e}")
# === END LOW VRAM HELPERS ===
...
def generation_all(
    ...
    # === LOW VRAM: swap shape->CPU, texture->GPU ===
    if args.low_vram_mode:
        print("[LOW_VRAM] Swapping models for texture generation...")
        offload_shape_to_cpu()
        move_texture_to_gpu()

    path_textured = tex_pipeline(mesh_path=path, image_path=image, output_mesh_path=text_path, save_glb=False)

    # === LOW VRAM: swap back — texture->CPU, shape->GPU ===
    if args.low_vram_mode:
        print("[LOW_VRAM] Swapping models back for next generation...")
        move_texture_to_cpu()
        restore_shape_to_gpu()
...
if __name__ == '__main__':
    ...
    # ====== PATCHED: Load shape FIRST, then texture to CPU (16GB VRAM fix) ======
    ...
    print("[LOW_VRAM] Step 1/2: Loading shape model to GPU...")
    ...
    # Now load texture pipeline — to CPU in low_vram_mode
    ...
    if not args.disable_tex:
        try:
            ...
            if args.low_vram_mode:
                print("[LOW_VRAM] Step 2/2: Loading texture model to CPU (saves ~12GB VRAM)...")
                offload_shape_to_cpu()
            ...
            if args.low_vram_mode:
                restore_shape_to_gpu()
                vram_free = torch.cuda.mem_get_info()[0] / 1024**3
                print(f"[LOW_VRAM] Init complete. Free VRAM: {vram_free:.1f} GB")

                # [MMGP] Apply mmgp to texture multiview pipeline
                try:
                    core_pipe = tex_pipeline.models["multiview_model"].pipeline
                    offload.profile(core_pipe, profile_type.HighRAM_LowVRAM)
                    print("[MMGP] Applied HighRAM_LowVRAM profile")
                except Exception as mmgp_err:
                    print(f"[MMGP] HighRAM_LowVRAM failed: {mmgp_err}")
                    try:
                        core_pipe = tex_pipeline.models["multiview_model"].pipeline
                        offload.profile(core_pipe, profile_type.LowRAM_LowVRAM)
                        print("[MMGP] LowRAM_LowVRAM fallback applied")
                    except Exception as mmgp_err2:
                        print(f"[MMGP] All profiles failed: {mmgp_err2}")
            HAS_TEXTUREGEN = True
